...
from old_files.rolling.rolling_catch import execute_catch, minimum_lead

CONFIG=json.loads((Path(__file__).parents[1]/'rolling'/'rolling_catch.config.json').read_text())
...
class CatchTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_move_only_does_not_wait_or_grab(self):
        from old_files.rolling.rolling_catch import move_to_prediction
