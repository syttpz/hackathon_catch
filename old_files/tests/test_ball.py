...
from old_files.red_ball.ball_tracking import BallCamera, Observation, Trajectory, detect_red_ball
from old_files.red_ball.ball_pickup import check_workspace, move_to_ball_and_grip
...
class PerceptionTests(unittest.TestCase):
    ...
    def test_live_overlay_reports_alignment(self):
        from old_files.red_ball.vision.live_ball import annotate
...
class CameraTests(unittest.IsolatedAsyncioTestCase):
    async def test_viewer_reconnects_after_stream_termination(self):
        ...
        from old_files.red_ball.vision.live_ball import stream
        ...
        with patch("old_files.red_ball.vision.live_ball.connect", AsyncMock(side_effect=[first, second])) as connect_mock, \
             patch("old_files.red_ball.vision.live_ball.Camera.from_robot", side_effect=[broken, good]), \
             patch("old_files.red_ball.vision.live_ball.asyncio.sleep", AsyncMock()):
...
class PickupTests(unittest.IsolatedAsyncioTestCase):
    async def run_pickup(self, execute, move_success=True, grabbed=True, shift=False):
        config = json.loads((Path(__file__).parents[2] / "ball_config.example.json").read_text())
        ...
        with patch("old_files.red_ball.ball_pickup.Arm.from_robot", return_value=arm), \
             patch("old_files.red_ball.ball_pickup.Camera.from_robot"), \
             patch("old_files.red_ball.ball_pickup.Gripper.from_robot", return_value=gripper), \
             patch("old_files.red_ball.ball_pickup.MotionClient.from_robot", return_value=motion), \
             patch("old_files.red_ball.ball_pickup.collect_track", AsyncMock(side_effect=[track, changed if shift else track, track])):
            try:
