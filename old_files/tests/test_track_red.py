...
from old_files.red_ball.track_red import (locate_target, estimate_distance, servo_step,
                             camera_tilt_deg, clamp_target, in_workspace,
                             red_blob_candidates, SmoothMove)

CONFIG = json.loads((Path(__file__).parents[1]/'red_ball'/'track_red.config.json').read_text())
