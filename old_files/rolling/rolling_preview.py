...
from old_files.rolling.can_tracking import red_can_candidates, can_center_height
from old_files.rolling.rolling_intercept import RollingFit, line_crossing
from old_files.rolling.table_plane import (
    fit_table_plane,
    point_on_offset_plane,
    InvalidProjection,
)
from old_files.rolling.rolling_catch import (
    execute_catch,
    minimum_lead,
    validate_catch_config,
    prepare_gripper,
    move_to_prediction,
)
...
def main():
    ...
    parser.add_argument('--config',type=Path,default=Path(__file__).with_name('rolling_catch.config.json'))
